"""Uber Eats collector — browser-based, no reliable public API found.

CONFIDENCE: medium-low, same caveat as wolt.py/glovo.py. Uber Eats often
gates full browsing behind a delivery-address prompt before showing
restaurant results — the search flow below may need an extra
"set address" step that isn't modeled here yet. Verify on the real server.
"""
...
SEARCH_URL = "https://www.ubereats.com/search"


class UberEatsCollector(PlatformCollector):
    ...
    async def _collect(self, page: Page, restaurant_name: str, city: str, max_reviews: int) -> list[Mention]:
        ...
        result_link = page.locator('a[href*="/store/"]').first
        ...
        review_cards = page.locator('[data-test*="review"], [class*="review"]')
...
if __name__ == "__main__":
    ...
    async def _manual_test() -> None:
        async with async_playwright() as pw:
            ...
            result = await UberEatsCollector().collect(browser, "Picasso", "Berlin", "DE")
