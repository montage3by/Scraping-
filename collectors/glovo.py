"""Glovo collector — browser-based, no reliable public API found.

CONFIDENCE: medium-low, same caveat as wolt.py — URL/selector guesses
based on general knowledge, not verified live. Glovo's web client is
city-scoped (glovoapp.com/{country}/{city}/...); the generic search entry
point below is a best guess, not confirmed. Run on the real server first.
"""
...
SEARCH_URL = "https://glovoapp.com/search"


class GlovoCollector(PlatformCollector):
    platform_id = "glovo"
    ...
    async def _collect(self, page: Page, restaurant_name: str, city: str, max_reviews: int) -> list[Mention]:
        ...
        review_cards = page.locator('[data-test-id*="review"], [class*="review"]')
...
if __name__ == "__main__":
    ...
    async def _manual_test() -> None:
        async with async_playwright() as pw:
            ...
            result = await GlovoCollector().collect(browser, "Picasso", "Tbilisi", "GE")
