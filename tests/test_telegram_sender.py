...
def test_send_message_dry_run_does_not_raise():
    send_message(chat_id="@dailyyolk_channel", text="Hello")


def test_send_message_dry_run_logs(caplog):
    with caplog.at_level(logging.INFO, logger="dailyyolk.telegram"):
        send_message(chat_id="@dailyyolk_channel", text="Hello world")
    assert any("DRY RUN" in r.message for r in caplog.records)
    assert any("@dailyyolk_channel" in r.message for r in caplog.records)


def test_send_document_dry_run_does_not_raise(tmp_path):
    fake_pdf = tmp_path / "report.pdf"
    fake_pdf.write_bytes(b"%PDF-fake")
    send_document(chat_id="@dailyyolk_channel", file_path=fake_pdf, caption="New report")
